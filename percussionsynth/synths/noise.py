...
class NoiseGenerator(nn.Module):
    ...
    """ Creates the time evolving impulse response of a FIR filter defined in freq domain.
        Input is a progression of frequency bands of shape [nb, frame_len, num_bands] """
    def amp_to_impulse_response(self, amp: torch.Tensor, target_size: int):
        
        # Treat time evolving amplitude bands as frequency responses and compute the IR.
        # Each amplitude band in frequency domain is a very narrow filter.
        # We breadth the bandwidth of the filter by multiplying the IR with a Hann window.
        # Then we convolve the IR with the noise source. 
    ...
    '''Overlaps and adds a filtered noise sequence with format [nb,frame_len,num_bandsß]
        Returns noise signals with format [b,n]'''
    ...
    ''' noise_bands(torch.Tensor): 
            A tensor with predicted filtered noise coefficients [nb, frame_len, num_bands] '''
