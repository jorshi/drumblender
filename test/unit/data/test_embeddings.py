...
from drumblender.data.embeddings import OpenL3


def test_openl3_is_none_raises_import_error_on_call(mocker):
    mocker.patch("drumblender.data.embeddings.openl3", None)
...
def test_openl3_asserts_on_incorrect_input_shape(mocker):
    mocker.patch("drumblender.data.embeddings.openl3")
    emb = OpenL3(sample_rate=44100)
...
def test_openl3_is_called_with_correct_arguments(mocker):
    mock = mocker.patch("drumblender.data.embeddings.openl3")
