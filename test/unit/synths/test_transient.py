...
import pywt
from percussionsynth.synths import WaveletTransform, InverseWaveletTransformFunctional
...
def test_wavelet_reconstruction():
    ...
    for i in range(num_levels):
        filt_pair["rec_lo"] = repeat(torch.tensor(wavelet_filters[2]),"f -> b f",b=batch_size)
        filt_pair["rec_hi"] = repeat(torch.tensor(wavelet_filters[3]),"f -> b f",b=batch_size)
